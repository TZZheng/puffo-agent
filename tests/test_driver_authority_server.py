...
from puffo_agent.agent.harness import acp_driver as acp_driver_module
...
async def test_constrained_lingtai_spawn_gets_only_the_issued_endpoint(
    ...
    monkeypatch.setattr(acp_driver_module, "spawn_framed_child", spawn)
    driver = AcpDriver()
    await driver._spawn(
        RuntimeSpec(
            "/workspace",
            executable="lingtai",
            launch_args=("acp", "--profile", "puffo-v0", "--runtime-id", "r1"),
            environment={DRIVER_AUTHORITY_FD_ENV: "caller-controlled", "SAFE": "1"},
        )
    )
